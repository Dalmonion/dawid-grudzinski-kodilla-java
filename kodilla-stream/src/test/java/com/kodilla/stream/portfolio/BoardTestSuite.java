...
public class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask() {
        ...
        List<TaskList> inProgressTasks = new ArrayList<>();
        inProgressTasks.add(new TaskList("In progress"));
        int allDays = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(Task::getCreated)
        ...
        //Then
        assertEquals(10, allDays / sizeOfList);
    }

    @Test
    void testAddTaskListAverageWorkingOnTaskHarderVersion() {
        // Given
        Board project = prepareTestData();
        ...
        List<TaskList> inProgressTasks = new ArrayList<>();
        inProgressTasks.add(new TaskList("In progress"));
        List <Integer> averageWorkingOnTaskList = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(Task::getCreated)
                .map(d -> (Period.between(d, LocalDate.now())).getDays())
        ...
        double average = IntStream.range(0, averageWorkingOnTaskList.size())
                .map(averageWorkingOnTaskList::get)
                .average().getAsDouble();
